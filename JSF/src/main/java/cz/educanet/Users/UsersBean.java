...
public class UsersBean implements Serializable {
    ...
    public void createUserAuthen(String name, String password, String email) {
        try (
                Connection c = DriverManager.getConnection("jdbc:mariadb://localhost:3309/jews?user=root&password=heslo");
                PreparedStatement pS = c.prepareStatement(
                        "INSERT INTO jews.user (fullName, email, hashedPassword, createdAt, updatedAt )" +
                                "VALUES (?, ?, ?, ?, ?)");
        ) {
            pS.setString(1, name);
